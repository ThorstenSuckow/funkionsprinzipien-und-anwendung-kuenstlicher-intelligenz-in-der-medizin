...
Bislang haben wir nur MCP-Netze betrachtet, bei denen wir die Zeiteinheiten nicht berücksichtigt haben. In Anlehnung an die Original-Arbeit wollen wir jetzt ein Netz Modellieren, das die **paradoxe Hitzeempfindung** <R>RHVK+82</R> simuliert: Das Phänomen, das bei kurzzeitiger Kühlung der Haut Hitzze empfunden wird.
...
Das nach <R>MP43, S. 105 Figure 1 (e)</R> aufgestellte Modell stellen wir folgendermaßen dar (vgl. <R>Fau94, S. 31 ff.</R>):
...
Es finden nun zwei Versuche statt: Im ersten wird versucht das paradoxe Hitzeempfinden zu simulieren. Hierzu wird zum Zeitpunkt $t-3$ der Kälterezeptor stimuliert, und kurz darauf unterbrochen zum Zeitpunkt $t - 2$. Die folgende Tabelle verdeutlicht die Signale, die an den jeweiligen Zellen zum Zeitpunkt $[t- 3, t]$ vorliegen:
