...
In <R>CBD+89</R> stellen _LeCun et al._ dieses Verfahren als **Convolutional Network** _LeNet-1_[^38] vor <R>CBB89, S. 13</R>, das äusserst erfolgreich durch Unterstützung von Backpropagation  handgeschriebene Postleitzahlen erkennt[^39]. Das Netz performt mit 30 Klassifizierungen pro Sekunde, lediglich die Normalisierung der Eingabedaten stellt einen Flaschenhals bei dem Prozess dar. Wird dieser berücksichtigt, werden 10-12 Ergebnisse pro Sekunde erzielt <R>CBD+89, S. 549</R>. <br />.

Die mathematische Basis der CNN ist die **Faltungsoperation**[^40]: Hierbei wird auf Eingabedaten ein **Kernel** (die _Faltungsmatrix_) angewendet, wobei das Ergebnis der Faltungsoperation eine feature map ist
...
[^38]: "Its architecture is a direct extension of the one proposed in LeCun (1989)." <R>CBD+89 S. 544</R> Iterationen von _LeNet_ durch _LeNet-4_ und _LeNet-5_. Die Architektur von _LeNet-5_ wird später angemessen zu der in Mitte der 90er Jahre zur Verfügung stehenden Technologie skaliert: "In 1989 a recognizer as complex as LeNet-5 would have required several weeks' of training, and more data than was available, and was therefore not even considered." <R>CBB+98, S. 15</R> Die Eingabedaten sind ein $32x32$ Pixel grosses Bild, ausserdem besitzt das Netz 7 Schichten (ohne Eingabeschicht) (_LeNet-1_: 3 verborgene Schichten <R>CBD+89, S. 544</R>). Eine vollständige Beschreibung der eindrucksvollen Architektur findet sich in <R>CBB+98, S. 7 ff.</R>
[^39]: Wobei die Bilder zu Matrizen der Dimension $16 x 16$ transformiert und als Eingabedaten in das Netz eingespeist werden. Die Transformation behält das Seitenverhältnis der Zeichen bei. Die Werte der Matrix an den Positionen $a_{ij}$ entsprechen Grauwerten mit einem normalisierten Wertebereich von $[-1, 1]$ <R>CBD+89, S. 542</R>
