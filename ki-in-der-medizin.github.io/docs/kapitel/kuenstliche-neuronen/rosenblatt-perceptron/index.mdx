...
Bereits 1954 wurden Versuche unternommen, lernfähige neuronale Netze zu modellieren <R>Ros62, S. 24, Abs. 2</R>[^10]. Erst 1958 schafft es ein Modell, eine Sensation auszulösen: Das Perzeptron  <R>AR88, S.90</R>[^11]. 1957 beschreibt es sein Schöpfer Frank Rosenblatt (1928 - 1971) in <R>Ros57</R> als Teil eines internen Forschungsprojektes des _Cornell Aeronautical Labors_: Die Forschungseinrichtung gehörte von 1946 bis 1972[^12] zu der Cornell Universität[^13], an der Rosenblatt 1950 seinen A.B und 1956 seinen Ph.D. gemacht hatte, und an der er bis zu seinem Lebensende[^14] als Psychologe und Neurobiologe forschen und lehren wird <R>Ehb71</R>.

In dem Forschungsprojekt "_Perceiving and Recognizing Automaton_" ging es um einen Apparat, der mittels einer Kamera geometrische Figuren erkennen und zuordnen kann. Ihr Verhalten hatte Rosenblatt zunächst auf einem IBM 704 Rechner simuliert<R>Ros60</R>, bevor sie Anfang der 60er Jahre als _Mark 1 Perceptron_ gebaut wurde: 400 Cadmiumsulfid-Photozellen auf einem 20x20 großen Raster angeordnet - dem **S-System** (**S** = _Sensory_) - speisten Signale in das **A-System** (**A** = _Association_) ein; dort wurden sie registriert und ausgewertet, und schlussendlich über das **R-System** (**R** = _Response_) ausgegeben (vgl. <R>Ros57, S. 4, f.</R>, <R>Ros58 S. 389 ff.</R> sowie <R>Bis06, S. 193 "Frank Rosenblatt" sowie S. 196 Figure 4.8</R>). Die  Maschine lernte im ersten Schritt durch die Unterstützung der Ingenieure, wie welche Formen zu interpretieren sind: Die Photozellen wurden aktiviert, und die erwartete Ausgabe wurde manuell festgelegt. Die Verbindungen zwischen den **S**-, **A**- und **R**-Units erinnert nicht nur von der Namensgebung her an biologische Neuronen, auch deren Struktur und Verschaltung wurde hier als Vorbild genommen[^15] <R>Ros62, s. 4, Abs. 2</R>.
...
DEFINITION 17: A perceptron is a network of S, A, and R units with a
variable interaction matrix V which depends on the
sequence of past activity states of the network. <R>Ros62, S. 83</R> (Definition, a, r, s S. 79 ff.)

DEFINITION 9: A simple A-unit is a logical decision element, which
generates an output signal if the algebraic sum of its
input signals, ocI , is equal or greater than a threshold
quantity, G > 0 . The output signal a * is equal to ¥- I
if ait > Q and 0 otherwise. If a * = + I , the unit
is said to be active. <R>Ros62, S. 81, "Definition 9"</R>

Bzgl. Def 9.
> The writer makes no claim to uniqueness in this respect. In particular, the neuron model employed is a
...
writer's predilection for a probabilistic approach is shared with such theorists as Ashby, Uttley, Minsky, MacKay, and von Neumann, among others. [Ros61, S. 5]
