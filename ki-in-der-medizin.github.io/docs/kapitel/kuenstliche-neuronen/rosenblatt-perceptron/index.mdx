...
Die Schritte werden so lange durchlaufen, bis für alle Trainingsdaten die Ausgabe korrekt ist, oder eine maximale Anzahl von Trainingsläufen erreicht wurde. Einen Trainingslauf nennt man dabei _Epoche_ <R>Fau94, S. 436, "Training epoch"</R>. Sind die Trainingsdaten linear separabel, _konvergiert_[^24] das Perzeptron nach einer endlichen Zahl von Epochen <R>MP17, S. 164</R>[^25], und ist danach in der Lage zu _generalisieren_ <R>Ert21, S. 202</R>.
...
Dies erreicht man mit einer sogenannten **bias unit**. Das Bias-Gewicht[^26] ist ein Wert, der zu der Gleichung aus **Definition 2.2.2** hinzuaddiert wird, und für eine Verschiebung der Ursprungsgeraden[^27] sorgt.
...
Mit $b$ als Teil der Eingabe folgt auch dessen Gewichtung $bw_{n+1} = w_{n+1}$. Mit $0$ als Schwellenwert wird dadurch eine Verschiebung der Trenngeraden entlang der $y$-Achse im $\R^2$ bzw. eine Verschiebung der Hyperebene im $\R^n$ ermöglicht<R>Ert21, S.215</R> [^28].

**Abbildung 2.2.6** Geometrische Interpretation der Perzeptron-Funktion im $\R^2$


#### Lernrate
Bevor wir uns eine Implementierung in Python anschauen, wollen wir noch kurz die **Lernrate** $\alpha$ erklären. Wie wir in diesem Abschnitt erfahren haben, berechnet die Lernregel die Gewichte auf Basis des _Fehlers_, also der Differenz von $\text{erwartete Ausgabe}$ und $\text{tatsächliche Ausgabe}$: Ist der Fehler $-1$, werden die Gewichte verringert, ist der Fehler 1, werden die Gewichte erhöht. Die Lernrate $\alpha$ ist der Koeffizient für die Gewichtsanpassung [^29]. Üblicherweise liegt $\alpha$ zwischen $0$ und $1$[^30] <R>Fau94, S. 61</R>.
...
[^28]: ein hilfreicher Überblick über die geometrischen Zusammenhänge ist in "Einführung in die Neuroinformatik" von Prof. Dr. G. Sommer https://www.informatik.uni-kiel.de/inf/Sommer/doc/Downloads/Skripte/neuroskript.pdf, S. 33 ff. zu finden (abgerufen 27.08.2023)
[^29]: Auch _Schrittweite_: vgl <R>GBC18, S. 93</R> und <R>RN09, S. 172</R>
[^30]: "$\alpha$ ist eine kleine Konstante" <R>RN09, S. 172, Abs. 2</R>. Gleiche Stelle beleuchtet das Für und Wider kleiner und großer Werte von $\alpha$
...
Für unsere Perzeptron-Implementierung in Python[^30] implementieren wir die Lernregel wie folgt:
...
[^30]: vollständiger Quellcode unter https://github.com/ThorstenSuckow/pylabs (abgerufen 25.08.2023)


## 2.2.4 Die XOR-Funktion

Wenn ein Perzeptron nicht konvergiert, kann es ausreichen, die Anzahl der Epochen zu erhöhen, damit passende Gewichtsvektoren gefunden werden.

import blob_success from "./img/blob_success.png";

<Embed figure={"2.2.8"} modeAware={false} title={"Ein Perzeptron mit großen Datenmengen (50 Einträg). Nach knapp 300 Trainingsschritten (in der 6ten Epoche) wird die Trenngerade gefunden. (Quelle: eigene)"}>
    <img src={blob_success} />
</Embed>

Allerdings kann es bereits bei wenigen Daten und beliebig großer Epochenzahl passieren, dass ein Perzeptron nicht konvergiert, nämlich wenn die Daten nicht linear separabel sind (vgl. auch Definition in Abschnitt **2.2.3**).
