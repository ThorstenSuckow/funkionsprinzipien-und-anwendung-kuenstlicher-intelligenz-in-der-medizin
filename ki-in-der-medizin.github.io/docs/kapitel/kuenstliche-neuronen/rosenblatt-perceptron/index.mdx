...
Mit $b$ als Teil der Eingabe folgt auch dessen Gewichtung $bw_{n+1} = w_{n+1}$. Mit $0$ als Schwellenwert wird dadurch eine Verschiebung der Trenngeraden entlang der $y$-Achse im $\R^2$ bzw. eine Verschiebung der Hyperebene im $\R^n$ ermöglicht<R>Ert21, S.215</R>.

#### Lernrate
Bevor wir uns eine Implementierung in Python anschauen, wollen wir noch kurz die **Lernrate** $\alpha$ erklären. Wie wir in diesem Abschnitt erfahren haben, berechnet die Lernregel die Gewichte auf Basis des _Fehlers_, also der Differenz von $\text{erwartete Ausgabe}$ und $\text{tatsächliche Ausgabe}$: Ist der Fehler $-1$, werden die Gewichte verringert, ist der Fehler 1, werden die Gewichte erhöht. Die Lernrate $\alpha$ ist der Koeffizient für die Gewichtsanpassung [^28]. Üblicherweise liegt $\alpha$ zwischen $0$ und $1$[^29] <R>Fau94, S. 61</R>.
...
[^28]: Auch _Schrittweite_: vgl <R>GBC18, S. 93</R> und <R>RN09, S. 172</R>
[^29]: "$\alpha$ ist eine kleine Konstante" <R>RN09, S. 172, Abs. 2</R>. Gleiche Stelle beleuchtet das Für und Wider kleiner und großer Werte von $\alpha$


## 2.2.3 Implementierung in Python

Wie bereits in Kapitel **2.1**, Abschnitt **2.1.3.3** wollen wir auch mit dem Rosenblatt Perzeptron boolesche Funktionen nachbilden. Das folgende Beispiel bezieht sich auf die **AND**-Funktion (vgl. **Tabelle 2.1.1** in Abschnitt **2.1.3.2**). Zunächst definieren wir zwei Mengen $M_+$ und $M_-$: Für Elemente in $M_+$ ist die erwartete Ausgabe $1$, für Elemente in $M_-$ ist sie $0$:
...
Mit der o.a. Implementierung konvergiert das Perzeptron innerhalb von 5 Epochen. In jeder Epoche werden 4 einzelne Daten trainiert, was insgesamt einer Anzahl von 20 Trainingsschritten entspricht. Die folgenden Tabellen stellen die Anpassungen dar (zugehörige Funktionsplots in **Abbildung 2.2.6**).
...
import epoch1_1 from "./img/AND/epochs.png";

<Embed
    figure={"Abbildung 2.2.6"}
    title={"Ein vollständiger Lauf über 5 Epochen für die AND-Funktion. Blaue und gestrichelt ist die Trenngerade, auf der senkrecht der Gewichtsvektor (orange) steht. Grün umrandete Punkte bedingen den Änderungsvektor (schwarz). (Quelle: eigene)"}
    modeAware={false}
>
    <img src={epoch1_1} />
</Embed>
...
Bei großen Datenmengen kann eine Anpassung der Epochen nach oben manchmal ausreichen (BILD BLOBS), damit ein Perzeptron sich anpasst
