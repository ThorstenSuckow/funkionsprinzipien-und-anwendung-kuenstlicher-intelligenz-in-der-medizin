...
} auszulösen: Das \textbf{Perceptron} (im folgenden ``Perzeptron``) [AR88:90]\footnote{
...
\begin{itemize}
    ...
    \item Punkte mit $x_1 - x_2 = 0$ \textit{auf} der Geraden liegen (im folgenden $\subset M_-$\footnote{
        Wenn die Hyperebene selbst im Halbraum enthalten ist, spricht man von einem _abgeschlossenen Halbraum_. (\url{https://de.wikipedia.org/wiki/Halbraum}, abgerufen 22.08.2023)
    })
\end{itemize}
...
Jeder Eintrag dieser Trainingsdaten ist einer erwarteten Ausgabe zugeordnet.
Der Algorithmus besteht aus folgenden Schritten (vgl. [RM87 S. 65] sowie [RN09 S. 842]):
...
Die Beziehung zu der Hebbschen Lernregel formulieren \textit{Arbib et al.}\footnote{
    vgl. hierzu auch \textit{Rosenblatt} : ``[ein Perzeptron hat] a tendency to develop 'cell assemblies' (in Hebb's sense), and these cell-assemblies tend to rival one another for dominance at all times. [Ros62:464, Abs. 2, Anführungszeichen doppelt i.O.]
}:
...
Die Lernrate $\eta$ ist der Koeffizient für die Gewichtsanpassung \footnote{
...
Eine Implementierung in Python des Rosenblatt-Perzeptrons findet sich in \ref{appendix:pythonperzeptron}.
