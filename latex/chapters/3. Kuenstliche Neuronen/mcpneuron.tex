...
Für die Konstruktion eines Netzes zur Modellierung von \textbf{XOR} hilft zunächst das in Abbildung~\ref{fig:mcpor} dargestellte Modell der \textbf{OR}-Funktion. $\Theta = 2$ ist erfüllt, wenn entweder $x_1$ oder $x_2$ ein erregendes Signal weiterleitet.
Allerdings wird $\Theta$ auch erfüllt, wenn jeweils $x_1$ \textit{und} $x_2$ gleichzeitig aktiv sind, denn dann ist $f_{xor}(g(X)) = 4$.
