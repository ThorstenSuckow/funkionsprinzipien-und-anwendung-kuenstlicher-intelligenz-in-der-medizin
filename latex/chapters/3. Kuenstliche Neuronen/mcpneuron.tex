...
\begin{figure}[h]
    ...
    \caption{Schematische Darstellung von MCP-Zellen (Quelle: in Anlehnung an~\cite[S. 105, Figure 1]{MP43})}
    \label{fig:mcpcell}
    \small{
...
\begin{figure}[h]
    ...
    \caption{Plot der Heaviside-Funktion $H\left(x\right)=\left\{x<0\ :\ 0,\ x\ge0\ :\ 1\right\}$. (Quelle: Eigene Darstellung)}
    \label{fig:heaviside}
\end{figure}
...
Die \textbf{disjunktive Normalform} (Gleichung~\ref{eq:gl-xordis}) und die \textbf{konjunktive Normalform} (Gleichung~\ref{eq:gl-xorcon}) von \textbf{XOR} liefern die Formel für das Netz, wobei insbesondere die disjunktive Normalform in Abbildung~\ref{fig:mcpxorf} erkennbar ist.\\
