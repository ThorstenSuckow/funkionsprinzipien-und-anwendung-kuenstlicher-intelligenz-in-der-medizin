...
\begin{figure}[h]
    ...
    \caption{Schematische Darstellung einer MCP-Zelle, nach \cite{MP43}}
    \small
...
\begin{definition} Ein MCP-Netz ist ein gerichteter Graph\footnote{
    Gerichtete Kanten eine MCP-Netzes nehmen eigentlich keine Gewichtung der Information vor [Roj93:40, ``2.4.1 Gewichtete und ungewichtete Netze``]. Sowohl \textit{Minsky} in~\cite[34]{Min67} als auch \textit{Rojas} in [Roj93:32] nutzen unter Berücksichtigung der \textit{absoluten Hemmung} nur die Summe der erregenden Signale und vergleichen diese mit $\Theta$ - dieser Vergleich findet nur statt, wenn \textit{kein} hemmendes Signal ankommt: Die Zelle liefert sonst direkt $0$ als Ausgabe zurück. [Fau94] nutzt\footnote{
